...
import { Tag, Plus, Loader2, CheckCircle, XCircle, Trash2, Store } from "lucide-react";
...
function OwnerCouponsContent() {
  ...
  const loadCoupons = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/coupons?ownerId=${user.uid}`);
      const data = await res.json();
      if (data.success) {
        setCoupons(data.coupons);
      }
    } catch (err) {
      logger.error("OWNER_COUPONS", "Erro ao carregar cupons", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleActive = async (coupon: CouponData) => {
    try {
      ...
      logger.error("OWNER_COUPONS", "Erro ao alterar status", err);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      logger.error("OWNER_COUPONS", "Erro ao excluir", err);
    }
  };
    ...
    <div className="condiments-page-container">
      <header className="page-header">
        <div className="header-text">
          <h1 className="page-title">Cupons do Owner</h1>
          <p className="page-subtitle">Gerencie os cupons promocionais da plataforma</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => !saving && setShowForm(false)}>
          <div className="bg-white rounded-2xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Store selector */}
              <div>
                <label className="block text-sm font-semibold mb-1">Lojas que aceitam o cupom</label>
                <p className="text-xs text-gray-500 mb-2">Se nenhuma for selecionada, vale em qualquer loja.</p>
                <div className="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-xl p-3">
                  {stores.map((store) => (
                    <label key={store.id} className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className="text-sm">{store.name || store.id}</span>
                    </label>
                  ))}
                  {stores.length === 0 && (
      ...
      {loading ? (
        ...
        <div className="space-y-3" style={{ marginTop: 24 }}>
          {coupons.map(coupon => {
            ...
            const storeNames = (coupon.storeIds || []).map((id) => storeMap[id]?.name || id).join(", ");
            return (
...
export default function OwnerCouponsPage() {
  return (
    <ErrorBoundary context="OwnerCouponsPage">
      <OwnerCouponsContent />
    </ErrorBoundary>
  );
}
